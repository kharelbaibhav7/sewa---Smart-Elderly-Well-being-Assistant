/**
 * SEWA Fall Detection Engine
 * Detects ground falls and bed falls using pose keypoint analysis.
 */
...
function torsoAngleFromVertical(leftShoulder, rightShoulder, leftHip, rightHip) {
...
export function createFallDetector(bedZone = null) {
  ...
  return {
    analyze(pose, frameWidth, frameHeight) {
      ...
      const bodyHeight = Math.abs(hipMid.y - shoulderMid.y);
      const isCollapsed = bodyHeight < frameHeight * 0.18;

      history.push({
        hipY: hipMid.y / frameHeight,
      ...
      if (isHorizontal && (isLow || isCollapsed)) {
        ...
        confidence = Math.min(
          1,
          (torsoAngle - TORSO_ANGLE_THRESHOLD) / 30 + (normalizedHipY - 0.5) * 0.8
        );
      }
      ...
      if (bedZone && history.length >= 4) {
        ...
        if (wasInBed && nowOutOfBed && yVelocity > BED_EXIT_VELOCITY && isHorizontal) {
          fallType = "bed";
...
export function drawPoseOverlay(ctx, pose, frameWidth, frameHeight, bedZone, result) {
